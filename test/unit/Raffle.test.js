...
!developmentChains.includes(network.name)
    ...
    : describe("Raffle Unit Tests", async function () {
          ...
          describe("Enter Raffle", function () {
              ...
              it("doesn't allow a player to enter when raffle is calculating", async function () {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  // for a documentation of the methods below, go here: https://hardhat.org/hardhat-network/reference
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.request({ method: "evm_mine", params: [] })
                  // we pretend to be a keeper to allow a change to state to be calculating for our comparison below
                  ...
                  await new Promise(async (resolve, reject) => {
                      raffle.once("WinnerPicked", async () => {
                          try {
                              ...
                              assert(recentWinner.toString(), accounts[1].address)
                              assert.equal(raffleState.toString(), "0")
